...
@Component({
  ...
  template: `
    <span
      class="inline-flex flex-row flex-nowrap items-center gap-1 overflow-hidden rounded-md border-[1px] border-slate-700 bg-slate-50 px-1 py-0.5 text-slate-700"
    >
      <span>{{ year() }}</span>
      @if (showFilter()) {
